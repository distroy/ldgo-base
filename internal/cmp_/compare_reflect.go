...
var (
	comparerTypes = &ldsync.Map[reflect.Type, typeInfo]{}
)
...
func refValueOf(v any) reflect.Value {
...
func getComparerTypeInfo(typ reflect.Type) typeInfo {
	if v, ok := comparerTypes.Load(typ); ok {
		return v
	}

	res := typeInfo{}
	defer func() {
		comparerTypes.Set(typ, res)
	}()
...
func compareReflectPointer(a, b reflect.Value) int {
	aa := a.Pointer()
	bb := b.Pointer()
	if r := CompareInteger(aa, bb); r == 0 || aa == 0 || bb == 0 {
		return r
	}
...
func compareReflectMap(a, b reflect.Value) int {
	// if r := CompareInteger(a.Len(), b.Len()); r != 0 {
	// 	return r
	// }
...
func compareReflectNumberLeftFloat(aa float64, b reflect.Value) int {
	...
	bb := b.Float()
	return CompareComparable(aa, bb)
}
...
func compareIntAndUint(a int64, b uint64) int {
	...
	return CompareInteger(uint64(a), b)
}
